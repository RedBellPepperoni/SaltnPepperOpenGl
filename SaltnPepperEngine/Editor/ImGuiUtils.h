...
namespace SaltnPepperEngine
{
	...
	namespace ImGuiUtils
	{
        ...
        bool Property(const char* name, Vector2& value, PropertyFlag flags);
        bool Property(const char* name, Vector2& value, float min = -1.0f, float max = 1.0f, PropertyFlag flags = PropertyFlag::None);
        bool Property(const char* name, Vector3& value, PropertyFlag flags);
        bool Property(const char* name, Vector3& value, float min = -1.0f, float max = 1.0f, PropertyFlag flags = PropertyFlag::None);
        bool Property(const char* name, Vector4& value, bool exposeW, PropertyFlag flags);
        bool Property(const char* name, Vector4& value, float min = -1.0f, float max = 1.0f, bool exposeW = false, PropertyFlag flags = PropertyFlag::None);
        bool PropertyTransform(const char* name, Vector3& vector, float width);

        bool Property(const char* name, Quaternion& value, PropertyFlag flags);

        bool PropertyDropdown(const char* label, const char** options, int32_t optionCount, int32_t* selected);

        void Tooltip(const char* text);

        void Tooltip(Texture* texture, const Vector2& size);
        void Tooltip(Texture* texture, const Vector2& size, const char* text);
        

        void Image(Texture* texture, const Vector2& size);
        void Image(CubeMap* texture, const Vector2& size);

        void TextCentred(const char* text);

        void SetTheme(Theme theme);

        bool BufferingBar(const char* label, float value, const Vector2& size_arg, const uint32_t& bg_col, const uint32_t& fg_col);
